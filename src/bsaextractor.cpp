...
BsaExtractor::BsaExtractor() : m_Organizer(nullptr) {}

bool BsaExtractor::init(MOBase::IOrganizer *moInfo) {
  m_Organizer = moInfo;
  moInfo->modList()->onModInstalled(
      [this](auto *mod) { modInstalledHandler(mod); });
  return true;
}

QString BsaExtractor::name() const { return "BSA Extractor"; }

QString BsaExtractor::localizedName() const { return tr("BSA Extractor"); }

QString BsaExtractor::author() const { return "Tannin & MO2 Team"; }

QString BsaExtractor::description() const {
  return tr(
      "Offers a dialog during installation of a mod to unpack all its BSAs");
}
...
QList<PluginSetting> BsaExtractor::settings() const {
  return {PluginSetting(
      "only_alternate_source",
      "only trigger bsa extraction for alternate game sources", true)};
}

bool BsaExtractor::extractProgress(QProgressDialog &progress, int percentage,
                                   std::string fileName) {
...
void BsaExtractor::modInstalledHandler(IModInterface *mod) {
  ...
  if (m_Organizer->pluginSetting(name(), "only_alternate_source").toBool() &&
      !(m_Organizer->modList()->state(mod->name()) &
        IModList::STATE_ALTERNATE)) {
    return;
  }

  if (QFileInfo(mod->absolutePath()) ==
      QFileInfo(m_Organizer->managedGame()->dataDirectory().absolutePath())) {
    QMessageBox::information(
        nullptr, tr("invalid mod name"),
        tr("BSA extraction doesn't work on mods that have the same name as a "
           "non-MO mod."
  ...
  if (archives.length() != 0 &&
      (QuestionBoxMemory::query(
           nullptr, "unpackBSA", tr("Extract BSA"),
    ...
    bool removeBSAs =
        (QuestionBoxMemory::query(
             nullptr, "removeUnpackedBSA", tr("Remove extracted archives"),
    ...
    foreach (QFileInfo archiveInfo, archives) {
      ...
      BSA::EErrorCode result = archive.read(
          archiveInfo.absoluteFilePath().toLocal8Bit().constData(), true);
      ...
      archive.extractAll(
          mod->absolutePath().toLocal8Bit().constData(),
      ...
      if (removeBSAs) {
        if (!QFile::remove(archiveInfo.absoluteFilePath())) {
          qCritical("failed to remove archive %s",
                    archiveInfo.absoluteFilePath().toUtf8().constData());
        }
      }
    }
  }
